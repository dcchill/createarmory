...
public class PipePistolItemRenderer extends GeoItemRenderer<PipePistolItem> implements RendersPlayerArms {
	public PipePistolItemRenderer() {
		super(new PipePistolItemModel());
	}
	...
	static {
		AnimationController.addModelFetcher(animatable -> {
			if (animatable instanceof PipePistolItem) {
				...
				BlockEntityWithoutLevelRenderer ister = new PipePistolItemRenderer();
	...
	@Override
	public void render(GeoModel model, PipePistolItem animatable, float partialTicks, RenderType type, PoseStack matrixStackIn, MultiBufferSource renderTypeBuffer, VertexConsumer vertexBuilder, int packedLightIn, int packedOverlayIn, float red,
