...
public class MacTenItemInHandTickProcedure {
			...
			if ((entity.getCapability(CreatearmoryModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new CreatearmoryModVariables.PlayerVariables())).Reload == true) {
				if ((entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == CreatearmoryModItems.MAC_TEN.get()) {
					...
					if (entity instanceof Player _player) {
						ItemStack _setstack = new ItemStack(CreatearmoryModItems.NINE_MM.get());
					...
					(entity instanceof LivingEntity _livEnt ? _livEnt.getOffhandItem() : ItemStack.EMPTY).getOrCreateTag().putBoolean("reloadKeyState", (true));
					if (entity instanceof Player _player) {
						ItemStack _setstack = new ItemStack(CreatearmoryModItems.NINE_MM.get());
						_setstack.setCount((int) itemstack.getOrCreateTag().getDouble("ammoCount"));
						ItemHandlerHelper.giveItemToPlayer(_player, _setstack);
